...
Workflow.prototype = {
  ...
  set source(externalSource) {
    ...
    if (isArray(externalSource)) {
      this.__source__ = externalSource;
    }
    else if (isPlainObject(externalSource)) {
      this.__source__ = externalSource;
    }
    else if (isNumber(externalSource)) {
      this.__source__ = times(parseInt(externalSource, 10));
    }
    else {
      this.__source__ = [];
    }

    this.__ordering__ = keys(this.__source__);
  },
  ...
  reflowAt(index, orderFn) {
    ...
    return this.reflow(function(requestedIndex) {
      if (requestedIndex === index) {
        ...
        return orderFn.call(flow, requestedIndex);
      }
    });
  },
  ...
  reflowIndex(requestedIndex) {
    ...
    const requestedItem = this.queryItem(requestedIndex);
    var i = reflow.length;
    var value;
    ...
    while (i--) {
      ...
      value = (reflow[i]).call(this, requestedIndex, requestedItem);
